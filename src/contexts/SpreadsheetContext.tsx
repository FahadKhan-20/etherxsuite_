...
import { CellState } from '../types/documentState';
...
export const SpreadsheetProvider: React.FC<{ children: React.ReactNode; initialData?: any }> = ({ children, initialData }) => {
  ...
  const cellData = React.useMemo(() => {
    ...
    Object.entries(activeSheet.cells).forEach(([key, cell]) => {
      const cellState = cell as CellState;
      if (cellState.value !== undefined) data[key] = cellState.value;
    });
    return data;
  }, [activeSheet.cells]);
  ...
  const cellFormats = React.useMemo(() => {
    ...
    Object.entries(activeSheet.cells).forEach(([key, cell]) => {
      const cellState = cell as CellState;
      if (cellState.style) formats[key] = cellState.style as CellFormat;
    });
    return formats;
  }, [activeSheet.cells]);
  ...
  const cellValidations = React.useMemo(() => {
    ...
    Object.entries(activeSheet.cells).forEach(([key, cell]) => {
      const cellState = cell as CellState;
      if (cellState.validation) validations[key] = cellState.validation;
    });
    return validations;
  }, [activeSheet.cells]);
